...
DECLARE
	...
	BEGIN
		...
		WHILE checker < counter LOOP
			IF Relation(checker)(2) = 101 THEN
				DBMS_OUTPUT.PUT_LINE(Relation(checker)(1) || ' ' || Relation(checker)(2) || ' ' || substr(Relation(checker)(3), 0, 20) || '･･･');
			END IF;
			checker := checker + 1;
		END LOOP;
	END;
	...
	BEGIN
		...
		FOR RECORD IN (
			SELECT	TO_CHAR(REL.ID_CR9060_PARENT, '000000') RESID,
					RES.ID_CR9061 RES_TYPE,
					RES.DH90301 R_NAME
			FROM CR9064 REL
			INNER JOIN CR9060 RES ON REL.ID_CR9060_PARENT = RES.ID_CR9060
			WHERE REL.ID_CR9060_CHILD = resourceId
			AND ID_CR9061 IN (12, 21, 52, 101)) LOOP

			DBMS_OUTPUT.PUT_LINE(RECORD.RESID || '  ' || mark || '  ' || substr(RECORD.R_NAME, 0, 20) || '･･･');
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('■■■■■■■■■■関連プロダクト調査 開始■■■■■■■■■■');

	-- 利用リソースの調査開始
	getRelation(id, '+');

	DBMS_OUTPUT.PUT_LINE('■■■■■■■■■■関連プロダクト調査 完了■■■■■■■■■■');
