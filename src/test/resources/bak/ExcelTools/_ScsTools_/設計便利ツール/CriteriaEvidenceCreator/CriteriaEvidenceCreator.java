 ...
 *
 * <P>2008/01/22 1.2<BR/>
 *    TEMPLATE処理時、対象テーブルID省略（BEFOREシートから自動取得）を不可にする。<BR/>
 *    AFTER処理時、BEFOREシートが消えてしまう不具合を修正。
 *    シート名のstatic取止め。
 * </P>
 * <P>2007/12/20 1.1<BR/> PREPAREの実装。シート名初期値変更。</P>
 * <P>2007/12/17 1.0<BR/> 新規作成。</P>
...
public class CriteriaEvidenceCreator extends EvidenceGeneratorImpl {
    ...
    class ExcelExporter {
        ...
        /**
         *
         * before <> null & after <> null →  処理後シート、差分シート出力（処理前シートに加えて）
