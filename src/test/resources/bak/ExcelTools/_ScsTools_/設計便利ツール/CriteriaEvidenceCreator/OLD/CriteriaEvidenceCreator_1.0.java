 ...
 *                   未入力のままEnterとした場合は初期値「処理前」となります。
 *
 *     - 処理タイプ：ツールに何をさせるのかを入力します。以下のコマンドがあります。
 *
 *                   PREPARE - (未実装)
 *                   BEFORE  - 処理前データの取得。DBデータのダンプを行います。
 *                   AFTER   - 処理後データの取得、処理前データとの差分取得を行います。
...
public class CriteriaEvidenceCreator extends EvidenceGeneratorImpl {
    ...
    /** 事前データ */
    private static String SHEET_NAME_PREPARE = "事前データ";

    /** 前シート名 */
    private static String SHEET_NAME_BEF = "処理前";

    /** 後シート名 */
    private static String SHEET_NAME_AFT = "処理後";

    /** DIFFシート名 */
    private static String SHEET_NAME_DIF = "差分";
    ...
    class CriteriaEvidenceManager extends DataBaseManager {
        ...
        public String createAfterEvidence() throws Exception {
            ...
            TrialInformation before =
                new ExcelImporter().doImport(this.getFullFilePath());
    ...
    class ExcelImporter {
        ...
        public TrialInformation doImport(String fullFilePath) throws Exception {
            ...
            try {
                ...
                if (file != null && file.exists()) {
                    ...
                    // 前シートの存在チェック
                    Sheet beforeSheet = book.getSheet(SHEET_NAME_BEF);
                    TrialInformation before =
                        ExcelSheetReader.readResultSheet(beforeSheet, manager
                            .getOutputPath(), new ArrayList());

                    if (before == null)
                        throw new Exception("そんなシートないです。[" + SHEET_NAME_BEF
                            + "]");
