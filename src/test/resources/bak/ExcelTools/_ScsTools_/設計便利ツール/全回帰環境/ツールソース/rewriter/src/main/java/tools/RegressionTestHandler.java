...
public class RegressionTestHandler {
    ...
    public static void main(String args[]) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if ("-murata-baseline-version".equals(args[i])) {
                murataBaselineVer = args[i + 1];
                continue;
            }
        }
        ...
        (new POMXMLReWriter(baseline == null ? DEFAULT_BASE_LINE: baseline, version == null ? DEFAULT_BASE_LINE_VERSION : version , murataBaselineVer)).walk(root);
