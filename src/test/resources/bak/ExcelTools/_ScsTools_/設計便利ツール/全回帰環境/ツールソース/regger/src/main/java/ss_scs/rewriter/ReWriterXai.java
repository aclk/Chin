...
public class ReWriterXai {
    ...
    public static String jdbcSettingFilePath = "tools/SAITO/jdbc.properties"; // デフォルトはtools/SAITOの下
    ...
    public static void main(String[] args) {
        ...
        Option jdbcPathOption =
            OptionBuilder.hasArg(true).withArgName("jdbcPath")
                .isRequired(false).withDescription(
                    "jdbc.propertiesパス(メッセージコピー等に使用)を指定").withLongOpt(
                    "jdbcPath").create("j");
        options.addOption(jdbcPathOption);
        ...
        String jdbcPath = commandLine.getOptionValue("j");
        if (jdbcPath != null) {
            jdbcSettingFilePath = jdbcPath;
        }
    ...
    public void rewrite(String target, String schemaName, String testType) {
        ...
        // 元となるjdbc.propertiesを取得する。
        Properties jdbcProperties = new Properties();
        InputStream ins = null;
        try {
            ins = new FileInputStream(jdbcSettingFilePath);
            jdbcProperties.load(ins);
            ins.close();
        } catch (Exception e) {
            jdbcProperties = null;
        }
        ...
        dw.walkAround(new File(target), ".*jdbc.*\\.properties", dip,
            new JdbcPropRewriteCommand(dbcSetting, jdbcProperties));
...
class DbConnectSetting {
    ...
    public boolean load(String filePath) {
        ...
        try {
            ...
            log.info("parameter/user : " + this.getUser() + " , password : "
                + this.getPassword() + " , jdbcUrl : " + this.getJdbcUrl());
                        ...
                        for (Object innerItem : pElement.elements()) {
                            if (innerItem instanceof Element) {
                                ...
                                if (cElement.getName().equals("property")) {
                                    ...
                                    // 接続文字列置換
                                    if ("jdbcUrl".equals(cElement.attribute(
                                        "name").getText())) {
                                        String jdbcUrl =
                                    ...
                                    // password置換
                                    if ("password".equals(cElement.attribute(
                                        "name").getText())) {
                                        String password =
...
class JdbcPropRewriteCommand implements FileRewriteCommand {
    ...
    public void rewrite(File targetFile) {
        ...
        try {
            ...
            // jdbc.propertiesがあった場合のみ処理
            // ※メッセージテーブルを書き換えてテストするやつが居るため、その対策
            if (jdbcProperties != null) {

                // rms.jdbc.url置換
                key = "rms.jdbc.url";
                newValue = jdbcProperties.getProperty(key);
                if (newValue != null) {
                    value = prop.getProperty(key);
                    if (value != null && !value.equals(newValue)) {
                        prop.setProperty(key, newValue);
