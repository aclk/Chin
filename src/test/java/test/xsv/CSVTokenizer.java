 ...
 * <p/>
 * An instance of <code>CSVTokenizer</code> behaves in one of two ways,
 * depending on whether it was created with the <code>returnSeparators</code>
 ...
 * maximal sequence of consecutive characters that are not separator.
 * </ul>
 * <p>
 * A <tt>CSVTokenizer</tt> object internally maintains a current position within
...
public class CSVTokenizer implements Enumeration {
	 ...
	 * tokens.
	 * <p/>
	 * If the <code>returnSeparators</code> flag is <code>true</code>, then the
	 * separator string is also returned as tokens. separator is returned as a
	 * string. If the flag is <code>false</code>, the separator string is
	 * skipped and only serve as separator between tokens.
	 *
	 * @param aString
	 ...
	 * @param theQuate
	 *            the quate (CSVTokenizer.SINGLE_QUATE,
	 *            CSVTokenizer.DOUBLE_QUATE, etc.).
	 */
	...
	/**
	 * Constructs a csv tokenizer for the specified string. The characters in
	 * the <code>theSeparator</code> argument are the separator for separating
	 ...
	 * uses the default separator set, which is
	 * <code>CSVTokenizer.SEPARATOR_COMMA</code>. Separator string themselves
	...
	public boolean hasMoreTokens() {
		return (this.current < this.record.length());
	}
	...
	public String nextToken() throws NoSuchElementException,
		...
		if (beforeStart == false) {
			current += this.separator.length();
		} else {
			beforeStart = false;
		}

		if (this.record.startsWith(this.quate, this.current)) {
			final StringBuffer token = new StringBuffer();
			String rec = this.record.substring(this.current
					+ this.quate.length());
			...
			this.current += (token.length() + this.quate.length() * 2);
			return token.toString();
		}

		final int end = this.record.indexOf(this.separator, this.current);
		if (end >= 0) {
			...
			this.current = this.record.length();
			return token;
		}
	}
	 ...
	 * of characters considered to be separator by this <tt>CSVTokenizer</tt>
	 * object is changed to be the characters in the string <tt>separator</tt>.
	...
	public int countTokens() {
		...
		final int preserve = this.current;
		final boolean preserveStart = this.beforeStart;
		while (this.hasMoreTokens()) {
			this.nextToken();
			count++;
		}
		this.current = preserve;
	...
	public static void main(String[] args) {
		CSVTokenizer csvt = new CSVTokenizer("this,is,a,test");
		while (csvt.hasMoreTokens()) {
			System.err.println(csvt.nextToken());
		}
